...
@Component({
  ...
  template: `
    <div class="glass rounded-2xl overflow-hidden hover-lift" data-aos="fade-up" [attr.data-aos-delay]="projectData.delay">
      <div class="relative overflow-hidden">
        <img [src]="projectData.image" [alt]="projectData.title" class="w-full h-48 object-cover transition-transform duration-500 hover:scale-110"/>
      ...
      <div class="p-6">
        <h4 class="text-xl font-bold mb-3">{{ projectData.title }}</h4>
        <p class="text-text-secondary mb-4 leading-relaxed">{{ projectData.description }}</p>
        <div class="flex flex-wrap gap-2 mb-4">
          @for (tech of projectData.technologies; track tech) {
            <span class="text-xs bg-accent/10 text-accent px-2 py-1 rounded">{{ tech }}</span>
          }
        </div>
        <span class="text-accent text-sm font-medium">{{ projectData.category }}</span>
      </div>
    </div>
  `
})
